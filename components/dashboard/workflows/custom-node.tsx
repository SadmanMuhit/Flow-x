...
const hasError = Boolean((data as any)?.hasError);
  return (
    <div className={`bg-[#1E293B] p-4 rounded-md border shadow-md min-w-45 relative cursor-pointer transition-colors` +
        (
            hasError ? "border-red-500 ring-2 ring-red-600" : isRunning ? "border-green-400 ring-2 ring-green-500 animate-pulse" : "border-[#1E293B] hover:border-green-400"
        )
    }>
      {hasError &&(
      ...
      {["left", "right", "top", "bottom"].map((pos)=>(
        <Handle key={pos} type={pos == "left" || pos == "top" ? "target" : "source"}
    position={
        ReactFlowPosition[
